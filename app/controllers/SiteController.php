...
class SiteController extends Controller
{
	...
	public function actionIndex()
	{
		...
		$userRoleId = User::model()->getCurrentUserRoleId();

		$renderOptions = null;
		if ($userRoleId == UserRole::USER_ROLE_BUYER) {
			// buyer
			$viewPath = 'buyer/index';
			$shopNames = User::model()->getShopNames();
			$renderOptions = array(
				'shopNames' => $shopNames,
			);
		} elseif ($userRoleId == UserRole::USER_ROLE_SELLER) {
			// seller
			$viewPath = 'seller/index';
		} else {
			throw new Exception('Unknown user role id');
		}
		$this->render($viewPath, $renderOptions);
	}
